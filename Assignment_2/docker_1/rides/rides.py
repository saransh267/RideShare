...
def create_ride():
	...
	response = make_request('http://172.17.0.1:8000/api/v1/users', data, headers, "GET")
	
	#print(response)
	if request.method != 'POST' \
						 '':
		return Response(json.dumps({"result": "Invalid method"}), 405)
	elif created_by not in response.json():
...
def join_ride(rideId):
	...
	#data = json.dumps({"table_name": "user", "column_names": ["*"], "where": where_clause})
	data = json.dumps({})
	response1 = make_request('http://172.17.0.1:8000/api/v1/users', data, headers, "GET")
